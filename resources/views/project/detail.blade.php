...
@section('content')
    ...
    <body
        ...
        @php
            $values = explode(',', $project_details->photo);
        @endphp
        @foreach ($values as $photo)
            <div class="mySlides vegas-slide vegas-transition-fade vegas-transition-fade-in"
                ...
                <div class="vegas-slide-inner"
                    style="background-image: url({{ $photo }}); background-color: rgb(0, 0, 0); background-position: center center; background-size: cover;">
                </div>
            </div>
        @endforeach
        ...
        <div class="project-info">
            <h2 class="project-title">{{ $project_details->title ?? '' }}</h2>
        </div>
